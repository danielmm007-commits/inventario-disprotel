(()=>{
 ...
 function estilos(){if(document.getElementById('ejecucionResumenCss'))return;const s=document.createElement('style');s.id='ejecucionResumenCss';s.textContent='.execQuick{display:grid;gap:8px;margin-top:12px}.execQuickMain,.execQuickSub{padding:11px 13px;border-radius:12px;font-weight:900}.execQuickMain{background:#eef6ff;border:1px solid #d7e6f7;font-size:16px}.execQuickSub{background:#f7fafb;border:1px solid #dde6ea;color:#425b67;font-size:13px}.datosOperativos{display:grid;grid-template-columns:1fr 1fr;gap:9px}.datosOperativos .opItem{background:#fff;border:1px solid #e1e8ec;border-radius:11px;padding:10px}.datosOperativos .opItem.full{grid-column:1/-1}.datosOperativos .opLabel{font-size:11px;color:#60737c;font-weight:800;text-transform:uppercase}.datosOperativos .opValue{font-weight:800;margin-top:3px;overflow-wrap:anywhere}@media(max-width:620px){.datosOperativos{grid-template-columns:1fr}.datosOperativos .opItem.full{grid-column:auto}.execQuickMain{font-size:14px}.execQuickSub{font-size:12px}}';document.head.appendChild(s)}
 ...
 function aplicar(){
   try{
     ...
     if(ctx&&!ctx.dataset.compacto){ctx.dataset.compacto='1';ctx.innerHTML=`<div class="execQuick"><div class="execQuickMain">📋 ${escLocal(ot)} · 👤 ${escLocal(nombre)}${ident?' · '+escLocal(ident):''}</div><div class="execQuickSub">🚐 ${escLocal(grupo)} · 👷 ${escLocal(tecnico)}</div></div>`}
