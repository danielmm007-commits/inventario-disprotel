(()=>{
  ...
  function ensureFrameLoadingCss(){
    ...
    const s=document.createElement('style');s.id='erpFrameLoadingCss';s.textContent=`
      .menuStage{position:relative!important}
      .menuFrame.erpPreparing{opacity:0!important;pointer-events:none!important}
      .menuFrame{transition:opacity .14s ease!important}
      .erpModuleLoader{position:absolute;inset:0;z-index:6;display:none;align-items:center;justify-content:center;background:#eef4f7}
      .erpModuleLoader.show{display:flex}
      .erpModuleLoader>div{display:flex;align-items:center;gap:10px;padding:12px 16px;border:1px solid #d7e5ec;border-radius:12px;background:#fff;color:#486474;font-size:12px;font-weight:800;box-shadow:0 8px 24px rgba(7,42,70,.08)}
      .erpModuleLoader i{width:16px;height:16px;border:2px solid #c9dce7;border-top-color:#1583b7;border-radius:50%;animation:erpSpin .65s linear infinite}
  ...
  function getLoader(frame){
    ...
    const stage=frame.closest('.menuStage')||frame.parentElement;if(!stage)return null;
    let loader=stage.querySelector('.erpModuleLoader');
    if(!loader){loader=document.createElement('div');loader.className='erpModuleLoader';loader.innerHTML='<div><i></i><span>Abriendo módulo…</span></div>';stage.appendChild(loader)}
    return loader;
  }
  ...
  function install(){
    ...
    if(title&&!btn){
      ...
      btn.addEventListener('click',e=>{
        ...
        else{const pinned=document.body.classList.toggle('erpSidebarPinned');localStorage.setItem('disprotel_sidebar_pinned',pinned?'1':'0')}
        paintToggle(btn);
      });
    }
  ...
  window.addEventListener('resize',()=>{if(!mq.matches)document.body.classList.remove('erpMobileMenuOpen');paintToggle(document.querySelector('.erpMenuToggle'))});
  ...
  const hookFrame=f=>{
    if(hooked.has(f))return;hooked.add(f);
    const srcObs=new MutationObserver(muts=>{if(muts.some(m=>m.attributeName==='src'))setFrameLoading(f,true)});srcObs.observe(f,{attributes:true,attributeFilter:['src']});
    f.addEventListener('load',()=>{compactFrame(f);revealWhenReady(f)});
    if(f.getAttribute('src')&&f.getAttribute('src')!=='about:blank')setFrameLoading(f,true);
    compactFrame(f);
  };
  const obs=new MutationObserver(()=>document.querySelectorAll('iframe.menuFrame').forEach(hookFrame));obs.observe(document.documentElement,{childList:true,subtree:true});
  setTimeout(()=>document.querySelectorAll('iframe.menuFrame').forEach(hookFrame),300);
})();