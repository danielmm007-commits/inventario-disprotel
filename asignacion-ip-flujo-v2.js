(()=>{
  ...
  window.confirmar=async(orden,cand)=>{
    ...
    try{const d=await api('confirm',{orden_id:orden,candidato_id:cand});inlineMsg(card,'✅ IP '+(d.ip||'')+' confirmada como definitiva.','ok');show('✅ IP confirmada.','ok');await cargar()}
  ...
  window.editarIpDetectada=async(sol,orden,ip)=>{
    ...
    try{await api('pause-edit',{solicitud_id:sol});editando.add(sol); aplicar();const inp=document.getElementById('manual-'+sol);if(inp){inp.value=ip;inp.focus();try{inp.setSelectionRange(0,inp.value.length)}catch{}}show('🔒 Scanner pausado solo para esta solicitud. Corrige la IP y confirma.')}
    catch(e){show(e.message,'err')}finally{syncPendiente.delete(sol)}
  };

  window.cancelarEdicionIp=async(sol)=>{
    if(syncPendiente.has(sol))return;syncPendiente.add(sol);
    try{await api('resume-edit',{solicitud_id:sol});editando.delete(sol);aplicar();show('▶️ Edición cancelada. El scanner seguirá verificando esta solicitud hasta que Fernando confirme.')}
    catch(e){show(e.message,'err')}finally{syncPendiente.delete(sol)}
  };
  ...
  window.manual=async(sol,orden)=>{
    ...
    try{const d=await api('assign-manual',{orden_id:orden,solicitud_id:sol,ip,observacion:obs});inlineMsg(card,'✅ IP '+(d.ip||ip)+' confirmada manualmente.','ok');show('✅ IP corregida y confirmada.','ok');editando.delete(sol);await cargar()}
    catch(e){inlineMsg(card,'❌ '+e.message,'err');show(e.message,'err')}
  };
  ...
  const etiquetaEvento=e=>{
    ...
    if(/SOLICIT/i.test(e))return '📨 SOLICITUD DE IP';
  ...
  const historialPorFecha=h=>{const grupos={};for(const x of h){const f=fechaClave(x.created_at);(grupos[f]||(grupos[f]=[])).push(x)}return Object.entries(grupos).map(([f,arr])=>`<details style="margin-top:10px"><summary style="cursor:pointer;font-weight:800">📅 ${esc(f)} · ${arr.length} evento${arr.length===1?'':'s'}</summary>${arr.map(cardHist).join('')}</details>`).join('')};
  const otFinalizada=x=>['COMPLETADA','CANCELADA','CANCELADA_EN_SITIO','NO_EJECUTADA_CLIENTE'].includes(up(x?.orden_estado));

  window.buscarHist=async()=>{
    try{
      const d=await api('ip-history',{busqueda:(document.getElementById('buscar')?.value||'').trim()});
      const a=d.asignadas||[],h=d.historial||[];
      document.getElementById('asignadas').innerHTML='<h3>IP actualmente asignadas</h3>'+ (a.map(x=>`<div class="hist"><span class="badge okb">✅ IP ACTUAL</span><div class="ip">${esc(x.ip_asignada||'—')}</div><b>${esc(x.id_orden)} · ${esc(up(x.cliente_nombre||''))}</b><div class="meta">Solicitada por ${esc(x.solicitado_por||'—')} · ${esc(fmt(x.solicitado_at))} · Grupo ${esc(x.grupo_asignado||'—')}</div><div class="meta">Confirmada por ${esc(x.asignada_por||'—')} · ${esc(fmt(x.asignada_at))}</div>${otFinalizada(x)?'<div class="msg ok"><b>🔒 OT FINALIZADA · SOLO LECTURA</b><br>La IP queda disponible únicamente para consulta.</div>':`<button onclick="mostrarCambioIp('${x.orden_id}','${esc(x.ip_asignada||'')}')">🔁 REASIGNAR IP</button><div id="cambio-${x.orden_id}"></div>`}</div>`).join('')||'<div class="muted">No hay IP actuales con ese criterio.</div>');
      document.getElementById('historial').innerHTML=`<details style="margin-top:14px"><summary style="cursor:pointer;font-weight:800">🗂️ HISTORIAL COMPLETO</summary><div class="muted" style="margin-top:6px">Consulta de auditoría de eventos de IP agrupados por fecha.</div>${historialPorFecha(h)||'<div class="muted">No hay eventos.</div>'}</details>`;
