(function(){
  ...
  window.guardarIpEditada=async function(){
    ...
    try{
      const d=await post(API_IP,'modify-ip',{orden_id:ordenId(),ip,motivo});
      await window.estadoIp();
      const texto=d.sin_cambios?'✅ IP verificada correctamente · sin cambios.':`✅ IP actualizada: ${d.ip_anterior} → ${d.ip}.`;
      show(texto);
