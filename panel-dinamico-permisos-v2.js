(()=>{
  ...
  function apply(profile,keys){
    ...
    const transferCard=[...document.querySelectorAll('.module')].find(card=>card.querySelector('h3')?.textContent?.trim()==='Transferencias');if(transferCard&&normalize(profileName)==='TECNICO'){const link=transferCard.querySelector('a.btn');if(link){link.href='solicitudes-transferencias-tecnico.html?v=5';link.textContent='Abrir solicitudes y transferencias →'}const description=transferCard.querySelector('p');if(description)description.textContent='Abastecimientos recibidos, transferencias desde tu bodega y solicitudes de baja.'}
    const summaryTransfer=summaries[1]?.querySelectorAll('.stat')?.[1];if(summaryTransfer&&normalize(profileName)==='TECNICO')summaryTransfer.onclick=()=>location.href='solicitudes-transferencias-tecnico.html?v=5';
    document.querySelectorAll('.menuAside button[data-href]').forEach(btn=>{if(normalize(profileName)==='TECNICO'&&/SOLICITUDES Y TRANSFERENCIAS/i.test(btn.textContent||''))btn.dataset.href='solicitudes-transferencias-tecnico.html?v=5'});
