(()=>{
  ...
  function apply(profile,keys){
    ...
    const inventoryCard=[...document.querySelectorAll('.module')].find(card=>card.querySelector('h3')?.textContent?.trim()==='Inventario');if(inventoryCard&&normalize(profileName)==='TECNICO'){const description=inventoryCard.querySelector('p');if(description)description.textContent='Consulta de solo lectura de los equipos y materiales disponibles en tu bodega operativa.';const link=inventoryCard.querySelector('a.btn');if(link){link.href='mi-inventario-tecnico.html?v=108';link.textContent='Ver mi inventario →'}}
