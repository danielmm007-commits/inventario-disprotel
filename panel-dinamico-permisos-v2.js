(()=>{
  ...
  function apply(profile,keys){
    ...
    const transferCard=[...document.querySelectorAll('.module')].find(card=>card.querySelector('h3')?.textContent?.trim()==='Transferencias');if(transferCard&&normalize(profileName)==='TECNICO'){const link=transferCard.querySelector('a.btn');if(link){link.href='solicitudes-transferencias-tecnico.html?v=7';link.textContent='Abrir solicitudes y transferencias →'}const description=transferCard.querySelector('p');if(description)description.textContent='Abastecimientos recibidos, transferencias desde tu bodega y solicitudes de baja.'}
    const summaryTransfer=summaries[1]?.querySelectorAll('.stat')?.[1];if(summaryTransfer&&normalize(profileName)==='TECNICO')summaryTransfer.onclick=()=>location.href='solicitudes-transferencias-tecnico.html?v=7';
