...
function quitarInicioSoporte(html){return String(html).replace(/<button onclick="iniciarSoporte\('[^']+'\)">🟠 INICIAR SOPORTE<\/button>/,'')}
window.jobHtml=function(o,disp=false,hist=false){
  ...
  if(tipoSoporte(o)&&estado==='EN_CAMINO')html=quitarInicioSoporte(html);
  ...
  const arrival=estado==='EN_CAMINO'?`<button type="button" class="gpsOpBtn gpsArrival" data-arrival-btn="${o.id}" onclick="registrarHitoGps('${o.id}','llegada',this)">📍 LLEGADA AL CLIENTE</button>`:'';
  ...
  const ops=`<div class="gpsOps" data-gps-ops="${o.id}" data-tipo-trabajo="${o.tipo_trabajo||''}"><div class="gpsOpsHead"><b>DESPLAZAMIENTO</b><span class="gpsOpsState">${estado.replaceAll('_',' ')}</span></div><div class="gpsOpsTimes"></div><div class="gpsOpsButtons">${start}${arrival}${reas}</div><div class="reasState" data-reas-state="${o.id}"></div></div>`;
...
async function refrescarHitos(orden){const box=document.querySelector(`[data-gps-ops="${orden}"]`);if(!box)return;try{const d=await gpsApi(orden,'hitos');const hs=d.hitos||[],map=Object.fromEntries(hs.map(x=>[x.tipo_evento,x]));const a=map.ACEPTACION,t=map.INICIO_TRASLADO,l=map.LLEGADA;const state=box.querySelector('.gpsOpsState'),times=box.querySelector('.gpsOpsTimes'),buttons=box.querySelector('.gpsOpsButtons');if(l)state.textContent='📍 EN SITIO';else if(t)state.textContent='🚗 EN CAMINO';const line=[];if(a)line.push(`Aceptada ${hora(a.capturado_at)}`);if(t)line.push(`Traslado ${hora(t.capturado_at)}${t.estado_gps!=='REGISTRADO'?' · sin GPS':''}`);if(l)line.push(`Llegada ${hora(l.capturado_at)}${l.estado_gps!=='REGISTRADO'?' · sin GPS':''}`);times.textContent=line.join('  ·  ')||'Sin hitos registrados';if(l&&box.dataset.tipoTrabajo==='SOPORTE_TECNICO'&&buttons&&!buttons.querySelector('.gpsBeginSupport')){buttons.querySelector(`[data-arrival-btn="${orden}"]`)?.remove();const b=document.createElement('button');b.type='button';b.className='gpsOpBtn gpsBeginSupport';b.textContent='🟠 INICIAR SOPORTE';b.onclick=()=>window.iniciarSoporte?.(orden);buttons.prepend(b)}}catch{}}
...
window.refrescarGpsTodos=refrescarTodos;
...
if(!document.getElementById('continuidadTecnicoLoader')){const s=document.createElement('script');s.id='continuidadTecnicoLoader';s.src='continuidad-tecnico-v1.js?v=20260901-2310';document.body.appendChild(s)}
})();