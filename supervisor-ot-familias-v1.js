(()=>{
  ...
  const p=chooser.querySelector('h2+p');if(p)p.textContent='Elige: instalación, soporte al cliente, reparación de troncal o intervención de nodo.';
  ...
  function block(type){
    ...
    if(type==='CAMBIO_EQUIPOS')return `<section class="multiBlock"><h4>🔄 Cambio de equipos</h4><div class="otIpHint" style="margin:0 0 10px">La oficina registra lo que conoce. El técnico confirmará en campo si realmente corresponde reemplazar el equipo y cuál es.</div><div class="multiGrid">${field('Equipo probable a revisar / cambiar','<input data-detail data-label="EQUIPO PROBABLE" placeholder="OPCIONAL · ROUTER, ONU, DECODIFICADOR...">')}${field('Serial actual','<input data-detail data-label="SERIAL ACTUAL" placeholder="OPCIONAL · SI YA SE CONOCE O ESTÁ ASOCIADO">')}${field('Motivo de la revisión / posible cambio *','<textarea data-detail data-required data-label="MOTIVO CAMBIO" placeholder="EJ.: REINICIOS, FALLA DE WI-FI, EQUIPO NO ENCIENDE..."></textarea>',true)}</div></section>`;
